...
2. **`schemas/tables.json`** — 각 테이블의 정확한 컬럼명과 타입. SQL 작성 시 컬럼명 확인에 사용한다.

예: "티켓 등록한 사용자 수"를 물으면
→ `tables_description.md`에서 `ticket_book` 테이블 확인
→ `schemas/tables.json`에서 `ticket_book`의 `user_id`, `created_at` 컬럼 확인
→ SQL 작성
